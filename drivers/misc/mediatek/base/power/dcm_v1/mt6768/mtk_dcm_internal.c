...
int mt_dcm_dts_map(void)
{
	...
	if (!dcm_ch0_emi_base) {
		dcm_pr_debug("error: cannot iomap emi ch0\n");
		return -1;
	}
	dcm_ch1_emi_base = (unsigned long)of_iomap(node, 3);
	if (!dcm_ch1_emi_base) {
		dcm_pr_debug("error: cannot iomap emi ch1\n");
		return -1;
	}
