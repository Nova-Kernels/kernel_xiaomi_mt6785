...
static inline bool susfs_is_current_proc_umounted_for_zygote_next(void) {
	return (likely(test_ti_thread_flag(&current->thread_info, TIF_PROC_UMOUNTED_FOR_ZYGOTE_NEXT)));
}
...
static inline bool susfs_is_current_proc_no_su(void) {
	return (likely(test_ti_thread_flag(&current->thread_info, TIF_PROC_NO_SU)));
}
